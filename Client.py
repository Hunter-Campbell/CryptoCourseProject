...
#This handles the communicaton loops for CBC and ECB (might work for OFB but havent gotten that far yet)
#Since ECB is not stateful while CBC is stateful, this function can take an optional paramater that isnt required
...
def communication_loop(server, cipher, *optional_decryption_cipher):
    ...
    if len(optional_decryption_cipher) <= 0:
        ...
        while True:
            ...
            ecb_cbc_encrypt_and_send(user_message_plaintext.encode(), server, cipher)

            received_cipher_text = recv_one_message(server)
            received_plain_text = ecb_cbc_decrypt_and_display(received_cipher_text, cipher)

            if(received_plain_text == "bye!"):
                ecb_cbc_encrypt_and_send("bye!".encode(), server, cipher)
        ...
        while True:
            ...
            received_cipher_text = recv_one_message(server)
            received_plain_text = ecb_cbc_decrypt_and_display(received_cipher_text, cipher, optional_decryption_cipher[0])

            if(received_plain_text == "bye!"):
                ecb_cbc_encrypt_and_send("bye!".encode(), server, cipher)
                #Give the server 1 second to receive the respons from the client
                time.sleep(1)
                break
